...
namespace LordKuper.OutfitManager
{
    ...
    public partial class Settings
    {
        ...
        {
            Common.UI.Tabs.DoTab(rect, 0, null, _generalContentHeight, ref _scrollPosition, DoGeneralTabContent, 0,
                null);
        }
        ...
        {
            var y = 0f;
            y += DoWorkTypeScoreFactorField(rect, out var remRect);
        ...
        /// <param name="remRect">The remaining rectangle after drawing the slider.</param>
        /// <returns>The height used by the slider field.</returns>
        private static float DoWorkTypeScoreFactorField(Rect rect, out Rect remRect)
        {
            return Fields.DoLabeledFloatSlider(rect, null, Resources.Strings.Settings.General.WorkTypeScoreFactorLabel,
                Resources.Strings.Settings.General.WorkTypeScoreFactorTooltip, ref _workTypeScoreFactor,
                WorkTypeScoreFactorMin, WorkTypeScoreFactorMax, 0.1f, out remRect);
        }
