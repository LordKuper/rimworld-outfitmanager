...
OM has no test project yet; this describes the target test setup. Intended reference: `<PackageReference Include="NUnit3TestAdapter" Version="6.2.0" />` in `Source/OutfitManager.Tests/OutfitManager.Tests.csproj` — the VSTest adapter that bridges NUnit tests to the VSTest platform. No source-level API; it is a discovery/execution plugin installed as a NuGet package.
...
- Despite the "3" in the name, the adapter runs both NUnit 3 and NUnit 4 tests. OM's target uses it to run NUnit 4.6.1 tests.
- Cannot run NUnit 2.x tests (not relevant here).
- Per the docs the adapter supports .NET Framework 3.5+ and modern .NET; OM's `net48` target is covered.
...
- No `.runsettings` or adapter-specific configuration planned; default discovery applies. Static-state tests rely on NUnit's `[NonParallelizable]` (see `nunit-4.6.1` reference) rather than adapter-level parallelism settings.
