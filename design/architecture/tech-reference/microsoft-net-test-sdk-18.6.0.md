...
OM has no test project yet; this describes the target test setup. Intended reference: `<PackageReference Include="Microsoft.NET.Test.Sdk" Version="18.6.0" />` in `Source/OutfitManager.Tests/OutfitManager.Tests.csproj` — marks the project as a runnable test project and supplies the test platform (the engine that runs tests and communicates with IDEs and the CLI). It has no compile-time API used directly in test code; its surface is MSBuild targets and the VSTest test host.
...
- Tests run on `net48` and depend on a RimWorld AssemblyResolve handler (see `nunit-4.6.1` reference); the SDK host is the process in which that resolver is registered at runtime.

## Known issues and workarounds
- None observed. The SDK is a passive platform dependency; failures surface as "no tests discovered/run", which usually trace to a missing or mismatched adapter (`NUnit3TestAdapter`) rather than the SDK itself.
